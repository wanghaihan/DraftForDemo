...
;( function( window ) {
	...
	SliderFx.prototype._init = function() {
		// the list of items
		this.itemsList = this.el.querySelector( '.sliderItems' );
	...
	SliderFx.prototype._navigate = function( dir ) {
		...
		if( dir === 'next' && this.curr < this.itemsCount - 1 ) {
			...
			this.curr = dir;
			this.direction = this.curr > this.old ? 'next' : 'prev';
		}

		if (this.curr === 1) {
			var text = document.querySelector('.imgP1Pro');
			text.className = 'icon imgP1Pro fadeInBottom';

			var pic = document.querySelector('.imgP1Links');
			pic.className = 'icon imgP1Links fadeInLeft';
		}
		// slide
		this._slide();
	};
